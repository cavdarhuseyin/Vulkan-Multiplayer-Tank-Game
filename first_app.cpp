...
namespace lve {
    ...
    net::ClientInputPacket FirstApp::buildLocalInput(std::uint32_t inputSequence) const {
        ...
        if (glfwJoystickIsGamepad(GLFW_JOYSTICK_1) &&
            ...
            float leftY = gamepad.axes[GLFW_GAMEPAD_AXIS_LEFT_Y];
            float leftX = gamepad.axes[GLFW_GAMEPAD_AXIS_LEFT_X];
            float rightX = gamepad.axes[GLFW_GAMEPAD_AXIS_RIGHT_X];
    ...
    void FirstApp::run() {
        ...
        while (!lveWindow.shouldClose()) {
            ...
            bool fpsToggleNow =
                glfwGetKey(lveWindow.getGLFWwindow(), GLFW_KEY_V) == GLFW_PRESS;
