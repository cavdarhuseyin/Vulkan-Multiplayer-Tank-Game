    ...
    namespace net {
        ...
        bool TankServer::collidesWithOtherTank(
            ...
            for (const auto& kv : players_) {
                ...
                if (!other.connected || !other.alive || other.playerId == selfPlayerId) continue;
        ...
        bool TankServer::hitTankWithMissile(
            ...
            for (const auto& kv : players_) {
                ...
                if (other.playerId == shooterPlayerId) continue;
        ...
        void TankServer::updateSimulation(float dt) {
            ...
            for (auto& kv : players_) {
                ...
                if (std::fabs(moveAxis) > 0.0001f) {
                    const ServerMeshCollision::Vec3 rayOrigin{ player.bodyPosX, player.bodyPosY - 1.0f, player.bodyPosZ };
                    const ServerMeshCollision::Vec3 rayDir{ bodyForwardX * moveAxis, 0.f, bodyForwardZ * moveAxis };
                    float hitDistance = (std::numeric_limits<float>::max)();
                    const bool hitsWorld = worldRayHit(rayOrigin, rayDir, 2.5f, hitDistance);
                ...
                if (player.missileActive) {
                    ...
                    const ServerMeshCollision::Vec3 missileOrigin{ player.missilePosX, player.missilePosY, player.missilePosZ };
                    const ServerMeshCollision::Vec3 missileDir{ player.missileVelX, player.missileVelY, player.missileVelZ };
                    float hitDistance = (std::numeric_limits<float>::max)();
                    ...
                    if (hitTankWithMissile(player.playerId, nextMissileX, nextMissileY, nextMissileZ, hitPlayerId)) {
                        player.missileActive = false;
                        player.missileDestroyed = true;
                        player.missileVelX = 0.f;
                        player.missileVelY = 0.f;
